...
| `efficiency_ratings.py` | SHADOW rating (never the on-air number): opponent-adjusted offense / defense / special teams from play-by-play — non-garbage PPA per play and success rate turned into "deserved points" per team-game, solved with the same ridge and the same preseason prior. Graded next to the on-air machine by `backtest.py`; weekly output `internal/shadow_ratings_weekN.json` and a "Shadow Ratings" sheet. |
| `totals_model.py` | SHADOW totals: opponent-adjusted pace times points per play from the shadow offense / defense ratings, calibrated on 2022–24; backtested against the closing total; writes a shadow score call (on-air margin over the modeled total) next to the on-air call in `score_tracker.json`. |
| `availability.py` | SHADOW availability layer: detects each team's starting quarterback from the play-by-play and flags a starter who did not throw last week; a manual injury file with a small CLI; a per-team points adjustment (favorites only, scaled by the number) shown beside the machine's line, never replacing it. |
| `playoff_sim.py` | Playoff Monte Carlo: remaining schedules from the machine's rating, conference title games with tiebreakers, a committee model fit to the 2014–2025 selection-Sunday rankings (rating, losses, strength of record, quality wins, champion flag, head-to-head; frozen in `playoff_committee_model.json`), 12-team field with the five highest-ranked champions and straight seeding, bracket to a title. Per team: playoff, bye, conference and national title odds and the most likely path. Replaces `playoff_sim_rough.py`. |
